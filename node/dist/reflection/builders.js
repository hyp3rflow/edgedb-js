            ...
            switch (exp.type) {
                case "named":
                    ...
                    if (exp.isDefault) {
                        ...
                        if (moduleKind === "esm") {
                            exportList.push(`${name}${exp.as != null ? ` as ${exp.as}` : ""}`);
                        }
                        else {
...
class CodeBuilder {
    ...
    resolveIdentRef(identRef, importsExports) {
        ...
        let prefix = "";
        if (ref.dir !== this.dir) {
            const mod = adapter_node_1.path.posix.basename(ref.dir, adapter_node_1.path.posix.extname(ref.dir));
            prefix = `_${mod}`;
            let importPath = adapter_node_1.path.posix.join(adapter_node_1.path.posix.relative(adapter_node_1.path.posix.dirname(this.dir), adapter_node_1.path.posix.dirname(ref.dir)), mod);
...
class DirBuilder {
    ...
    async write(to, params) {
        const dir = adapter_node_1.path.normalize(to);
        for (const [fn, builder] of this._map.entries()) {
            if (builder.isEmpty()) {
                continue;
            }
            const dest = adapter_node_1.path.join(dir, fn);
            const destDir = adapter_node_1.path.dirname(dest);
            if (!(await (0, adapter_node_1.exists)(destDir))) {
                await adapter_node_1.fs.mkdir(destDir, { recursive: true });
            }
            const forceDefaultExport = fn.startsWith("modules/");
            await adapter_node_1.fs.writeFile(dest + params.fileExtension, builder.render({
                mode: params.mode,
