...
function validateCredentials(data) {
    ...
    if (certData != null) {
        ...
        if (caData != null && certData !== caData) {
            throw new Error(`both 'tls_ca' and 'tls_cert_data' are defined, ` +
    ...
    if (tlsSecurity != null &&
        ...
        throw new Error(`\`tls_security\` must be one of ${conUtils_1.validTlsSecurityValues
            .map(val => `"${val}"`)
            .join(", ")}`);
    }
    if (verifyHostname &&
        ...
        throw new Error(`both 'tls_security' and 'tls_verify_hostname' are defined, ` +
            `and are not in agreement`);
    }
