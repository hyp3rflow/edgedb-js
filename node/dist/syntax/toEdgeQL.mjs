import { Duration, LocalDate, LocalDateTime, LocalTime, RelativeDuration, } from "edgedb";
import { Cardinality, ExpressionKind, isArrayType, isNamedTupleType, isObjectType, isTupleType, OperatorKind, TypeKind, util, } from "../reflection/index";
...
function renderEdgeQL(_expr, ctx, renderShape = true, noImplicitDetached = false) {
    ...
    if (expr.__kind__ === ExpressionKind.With) {
        ...
        return `(${withBlock}INSERT ${renderEdgeQL(expr.__expr__, ctx, false, true)} ${shapeToEdgeQL(expr.__shape__, ctx)})`;
    }
